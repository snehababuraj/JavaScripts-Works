...
// total number of filims

console.log(filims.length);

// movies whose runtime below 140
var runtimeFilter=filims.filter(f=>f.runtime<Number(140)).map(f=>f.title)
// console.log(runtimeFilter);


// movie with highest run time
var sortRuntime=filims.sort((f1,f2)=>f2.runtime-f1.runtime)
var highestRuntime=sortRuntime.map(f=>f.title)
// console.log(highestRuntime);
console.log(highestRuntime[0]);

// movie with lowest run time
var sortRuntime=filims.sort((f1,f2)=>f1.runtime-f2.runtime)
var lowestRuntime=sortRuntime.map(f=>f.title)
// console.log(lowestRuntime[0]);

// yearwise movie count

var yc={}

for(let filim of filims){
...
var nestedArray=Object.entries(yc)   //reduce map cannot apply on objects
var yearFilter=nestedArray.reduce((o1,o2)=>o1[1]>o2[1]?o1:o2)
// console.log(yearFilter);


// Action movie names
var genreFilter=filims.filter(f=>f.genres.includes("Action")).map(f=>f.title)
//  console.log(genreFilter);

// movie with genre either Action or Drama
var genreFilter=filims.filter(f=>f.genres.includes("Action" || "Drama")).map(f=>f.title)
// console.log(genreFilter);

var highestRunTimeMovie=filims.reduce((m1,m2)=>Number(m1.runtime)>Number(m2.runtime)>Number(m2.runtime)?m1:m2)
